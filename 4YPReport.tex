...
\begin{document}
	...
	\clearpage
	\newpage
	\includepdf[pages={1}]{DeclarationofAuthorship}
	\newpage
	\begin{center}
...
The second and third rows show the point clouds after being input to the 3D reconstruction system. The results naturally depend on some hyperparameters, as explained in Section \ref{sec:Borg}. This figure shows results for an edge-length equal to 0.2m. It can be seen that the surfaces are generally rough and noisy, due to imperfections in the laser scan data and in the inaccuracies associated with the SLAM solution.
	
The fourth and fifth rows show the results of regularizing the system using a $\lambda$ factor of 0.1 and number of iterations equal to 50. It is clear that by performing this regularization, many unconnected surfaces were removed, which was interpreted as noisy measurements due to the high variational components. The surfaces that remained are mostly long connected walls and roofs, which are significantly smoother than the unregularized reconstruction. It can also be seen that some of the sharp corners were smoothed. This is expected as the TV regularizer penalizes high variational profiles, which end up smoothening corners.
...
\begin{figure}
\centering
\caption*{\textbf{Oxford's Information Engineering Building Dataset}}
\vspace{-20pt}
\captionsetup[subfigure]{labelformat=empty}
\begin{tabular}{ccc}
...
In conclusion, the results that can be obtained using only LiDAR data are impressive. However, there is still much work to be done in order to achieve human-level performance. This is a promising time for the evolution of perception systems, and it is expected that new methods to produce better results will arise at an increasing pace.
	...
	\clearpage
	\newpage
	\includepdf[pages={1}]{RiskAssessment.pdf}
